...
class QueryStatePlugin():
    
    def __init__(self, query, pstate, initial_position = 0) -> None:
        self.input = query
        #self.pstate = pstate #need this?
    ...
    def handle_send(self, buff_addr, buff_length, pstate):
        ...
        if self.is_done_membership():
            self.probing_symbolic_var = pstate.memory.read(buff_addr, buff_length)
            status, models = pstate.solve(self.probing_symbolic_var.getAst())
            self.probing_results = models
            self.probing_result_type = 'SEND'
            self.done_probing = True
            self.probed_symbol = self.process_new_symbol() # TODO
            return
        ...
        if self.input[self.position].type == 'SEND':
            ...
            for (k, v) in predicate.items():
                ...
                if offset >= buff_length:
                    continue
                pstate.push_constraint(pstate.memory.read(buff_addr+offset, 1) == value)
            # print("Membership query SEND position: {} out of {}".format(self.position, len(self.input)))
            self.position = self.position + 1
        else:
            pstate.push_constraint(False)

    def handle_recv(self, buff_addr, buff_length, pstate):
        if self.done_probing:
            return
        ...
        '''
        if self.is_done_membership(): # TODO
        ...
        if self.input[self.position].type == 'RECEIVE':
            #sym_var = self.state.solver.BVS("x", buff_length * 8) # TODO
            #self.state.memory.store(buff_addr, sym_var)
            predicate = self.input[self.position].predicate
            for (k, v) in predicate.items():
                offset = int(k)
...
# function for hooking send/recieve symbols - register_function_callback(func_name: str, callback: tritondse.callbacks.AddrCallback)→ None
# callback should get (self, se : SymbolicExecutor, pstate : ProcessState, addr : int)